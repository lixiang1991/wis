...
class LogManager(object):
    ...
    def __init__(self):
        try:
            logs_dir = config_manager.get_config_values("python-log", "dir")
        except:
            logs_dir = os.path.join(utils.get_project_path(), 'logs')
        try:
            f_levl = config_manager.get_config_values("python-log", "f_level")
        except:
            f_levl = 'DEBUG'
        try:
            c_levl = config_manager.get_config_values("python-log", "ch_level")
        except:
            c_levl = 'DEBUG'
        ...
        maxFileSize = int(config_manager.get_config_values("python-log", "maxBytes"))
        #日志总数到了限制以后删除最旧的日志
        backUp = int(config_manager.get_config_values("python-log", "backupCount"))
        #设置控制台的输出级别，与文件输出区分开
        ...
        debug_rotatingFileHandler = RotatingFileHandler(debug_file_name, maxBytes=maxFileSize,
