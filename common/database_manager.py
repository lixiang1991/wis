...
from common.error_manager import DatabaseError
...
class Database(object):
    ...
    def __init__(self):
        ...
        if self.conn_use_return:
            ...
            for i in range(self.db_queue.maxsize):
                try:
                    conn = DatabaseConnection(self.conn_count)
    ...
    def get_connection(self, try_count=1):
        ...
        try:
            result = self.db_queue.get(timeout=0.1)
...
class DatabaseConnection(object):
    ...
    def __init__(self, conn_count=0):
        """
        数据库初始化，从配置文件读取主机名、端口号、用户名、密码、数据库名称、字符编码等数据库连接相关信息
        """
        self.conn = None
        self.host = config_manager.get_config_values('mysql', 'host')
        self.port = config_manager.get_config_values('mysql', 'port')
        self.user = config_manager.get_config_values('mysql', 'user')
        self.passwd = config_manager.get_config_values('mysql', 'password')
        self.db_name = config_manager.get_config_values('mysql', 'database_AI')
        self.charset = config_manager.get_config_values('mysql', 'charset')
        self.connect()  # 连接数据库
    ...
    def close(self):
        ...
        :return:
        """
        logger.debug('线程%s：正在关闭数据库连接...' % self.thread_name)
    ...
    def ping(self):
        result = self.fetchone('select id from file limit 1')
